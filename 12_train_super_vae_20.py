...
def create_model(input_dim):
    ...
    l = Dropout(0.7)(input_l)

    l = Dense(units=1280, activation=LeakyReLU(), kernel_initializer='he_uniform')(l)
    l = BatchNormalization()(l)

    l = Dense(units=640, activation=LeakyReLU(), kernel_initializer='he_uniform')(l)
    l = BatchNormalization()(l)

    l = Dense(units=320, activation=LeakyReLU(), kernel_initializer='he_uniform')(l)
    l = BatchNormalization()(l)

    l = Dense(units=160, activation=LeakyReLU(), kernel_initializer='he_uniform')(l)
    l = BatchNormalization()(l)

    l = Dense(units=80, activation=LeakyReLU(), kernel_initializer='he_uniform')(l)
    l = BatchNormalization()(l)

    #Latent space
    z_mean = Dense(units=20, name='z-mean')(l) # Mean component
    z_log_sigma = Dense(units=20, name='z-log-sigma')(l) # Standard deviation component
    ...
    # Decoder
    latent_space = Input(shape=(20,), name='input-z-sampling')
    l = Dropout(0.3)(latent_space)
    ...
    out = Dense(input_dim, name='decoder-output-layer')(l)

    # Create decoder model
    decoder = Model(latent_space, out, name='decoder')

    # Regression 
    reg_latent_space = Input(shape=(20,), name='input-z-sampling-regression')
    l = Dropout(0.5)(reg_latent_space)
    ...
    vae.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.001), loss={'supervised': 'mse'})
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(output+'all_training.png')
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(output+'vae_training.png')
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(output+'supervised_training.png')
...
plt.grid(True)
plt.savefig(output+'supervised_regression.png')
...
plt.xlabel('original values')
plt.grid(True)
plt.savefig(output+'vae_reconstruction.png')
...
with open(output+'performance_values.txt', 'w') as f:
...
vae.save(output+'supervised_model_20.h5')
encoder.save(output+'supervised_encoder_20.h5')
decoder.save(output+'supervised_decoder_20.h5')
vae_reg.save(output+'supervised_reg_20.h5')
