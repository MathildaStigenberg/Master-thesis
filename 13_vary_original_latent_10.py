...
for l in range(0,latent.shape[1]):
    ...
    for i, g in enumerate(genes):
        ...
        genes_slope[g] = slope_gene
    sorted_slope_genes = OrderedDict(sorted(genes_slope.items(), key=lambda item: item[1], reverse=True))
    low, high = np.percentile(list(sorted_slope_genes.values()),[10,90])
    fig1, ax1 = plt.subplots()
    fig2, ax2 = plt.subplots()
    for k, v in sorted_slope_genes.items():
        if v <= low:
            gene_exp = recon[:,gene_index[k]]
            corr, pvalue = pearsonr(latent_values,gene_exp)
            range_exp = np.max(gene_exp)- np.min(gene_exp)
            sorted_bottom_genes_lv[l].appendleft((k, v, corr, pvalue, range_exp))
        if v >= high:
            gene_exp = recon[:,gene_index[k]]
            corr, pvalue = pearsonr(latent_values,gene_exp)
            range_exp = np.max(gene_exp)- np.min(gene_exp)
            sorted_top_genes_lv[l].append((k, v, corr, pvalue, range_exp))
...
for i in range(0,latent.shape[1]):
    unique_up[i] = top_genes_lv[i].difference(top_genes_lv[(i+1)%10], top_genes_lv[(i+2)%10], top_genes_lv[(i+3)%10], top_genes_lv[(i+4)%10], top_genes_lv[(i+5)%10], top_genes_lv[(i+6)%10], top_genes_lv[(i+7)%10], top_genes_lv[(i+8)%10], top_genes_lv[(i+9)%10])
    unique_down[i] = bottom_genes_lv[i].difference(bottom_genes_lv[(i+1)%10], bottom_genes_lv[(i+2)%10], bottom_genes_lv[(i+3)%10], bottom_genes_lv[(i+4)%10], bottom_genes_lv[(i+5)%10], bottom_genes_lv[(i+6)%10], bottom_genes_lv[(i+7)%10], bottom_genes_lv[(i+8)%10], bottom_genes_lv[(i+9)%10])
...
table.to_pickle(output+'pathways_table_10.pkl')
